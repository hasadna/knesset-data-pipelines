...
class SQLTempTableDumper(SQLDumper):
    ...
    def initialize(self, parameters):
        if os.environ.get('DUMP_TO_SQL'):
            ...
            super(SQLTempTableDumper, self).initialize(parameters)

    def handle_resource(self, resource, spec, parameters, datapackage):
        if os.environ.get('DUMP_TO_SQL'):
            return super(SQLTempTableDumper, self).handle_resource(resource, spec, parameters, datapackage)
        else:
            return resource
